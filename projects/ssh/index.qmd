...
[Service]
Type=oneshot
ExecStart=/bin/python3 -c "import subprocess; localip = subprocess.check_output('ip route get 8.8.8.8', shell=True).decode('utf-8'); privateip = localip.split(); print(privateip[6]); subprocess.run(f'echo url=\'https://www.duckdns.org/update?domains=domainname&token=duckdnstoken&ip={private_ip[6]}\' | curl -k -K -', shell=True)"
...
Okay, I decided to create a proper python script. 

```{.python filename="/usr/bin/duckdns"}
#/usr/bin/python3
import subprocess
localip = subprocess.check_output('ip route get 8.8.8.8', shell=True).decode('utf-8')
privateip = localip.split()
print(privateip[6])
subprocess.run(f"echo url='https://www.duckdns.org/update?domains=domainname&token=duckdnstoken&ip={private_ip[6]}' | curl -k -K -", shell=True)

urllib.request.urlopen(f'https://www.duckdns.org/update?domains=domainname&token=duckdnstoken&ip={private_ip[6]}')
```
